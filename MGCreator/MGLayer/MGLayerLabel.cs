...
namespace MGCreator
{
    public class MGLayerLabel : MGLayer
    {
        ...
        public MGLayerLabel(MGForm m) : base(m)
        {
            ...
            m_Fill = MG_COL.White;
            m_FillOpacity = 100;
            m_Dotstyle = DotStyles.None;
            m_TextMargin = new Padding(5, 0, 5, 0);
            m_DotIsLeft = true;
            m_Dot = MG_COL.White;
            m_DotSize = new Size(20, 20);
        }
        // ***************************************************************************
                ...
                if (m_Dot != MG_COL.Transparent && m_Dotstyle != DotStyles.None)
                {
                    ...
                    switch (m_Dotstyle)
                    {
                        ...
                        case DotStyles.TriangleTop:
                            points = new PointF[3];
                            points[0] = new PointF(rr.Left + (float)rr.Width / 2, rr.Top);
                            points[1] = new PointF(rr.Right, rr.Bottom);
                            points[2] = new PointF(rr.Left, rr.Bottom);
                            g.FillPolygon(sb, points);
                            break;
                        ...
                        case DotStyles.TriangleLeft:
                            ...
                            points[1] = new PointF(rr.Right, rr.Top);
                            points[2] = new PointF(rr.Right, rr.Bottom);
                            g.FillPolygon(sb, points);
                            break;
                    }
                }
        ...
        public override JsonObject ToJson()
        {
            ...
            jn.SetValue("Text", m_Text);
            jn.SetValue("Alignment", (int)Alignment);
            jn.SetValue("Dotstyle", (int)m_Dotstyle);
            jn.SetValuePadding("TextMargin", m_TextMargin);
            jn.SetValue("DotIsLeft", m_DotIsLeft);
            jn.SetValueSize("DotSize", m_DotSize);
            jn.SetValue("Dot", (int)m_Dot);
        ...
        public override void FromJson(JsonObject jo)
        {
            ...
            float sz = 12;
            if (jn.GetFloat("FontSize", ref sz) == false) ret = false;
            int st = 12;
            if (jn.GetInt("FontStyle", ref st) == false) ret = false;
            m_Font = new Font(n, sz, (FontStyle)st);

            if (jn.GetInt("Alignment", ref st))
            {
            ...
            int v = 0;
            if (jn.GetInt("Dotstyle", ref v))
            {
                m_Dotstyle = (DotStyles)v;
            }
            else
            {
                ret = false;
            }
            if (jn.GetStr("Text", ref m_Text) == false) ret = false;
            if (jn.GetPadding("TextMargin", ref m_TextMargin) == false) ret = false;
            if (jn.GetBool("DotIsLeft", ref m_DotIsLeft) == false) ret = false;
            if (jn.GetSize("DotSize", ref m_DotSize) == false) ret = false;
            if (jn.GetInt("Dot", ref v))
            {
                m_Dot = (MG_COL)v;
            }
            else
