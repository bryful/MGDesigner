...
namespace BRY
{
	public class PrefFile
	{
		...
		// *********************************
		public void Add(string key,JsonObject jo)
		{
			m_data.Add(key, jo);
		}
		// *********************************
		public void AddArray(string key, JsonArray jo)
		{
			m_data.Add(key, jo);
		}
		public JsonObject? Data(string key)
		{
			JsonObject? ret = null;
			if (m_data.ContainsKey(key))
			{
				if (m_data[key] is JsonObject)
				{
		...
		public JsonArray? Array(string key)
		{
			...
			if (m_data.ContainsKey(key))
			{
				if (m_data[key] is JsonArray)
				{
					arr = m_data[key].AsArray();
				}
			}
			return arr;
		}
